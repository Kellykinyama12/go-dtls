...
func GenerateEncryptionKeys(masterSecret []byte, clientRandom []byte, serverRandom []byte, keyLen int, ivLen int) (*EncryptionKeys, error) {
...
func GenerateMasterSecret(preMasterSecret []byte, clientRandom []byte, serverRandom []byte) ([]byte, error) {
...
func GenerateKeyingMaterial(masterSecret []byte, clientRandom []byte, serverRandom []byte, length int) ([]byte, error) {
...
func generateAEADAdditionalData(Epoch int, SequenceNumber []byte, ContentType byte, Version int, payloadLen int) []byte {
	var additionalData [13]byte
	binary.BigEndian.PutUint16(additionalData[:], uint16(Epoch))
	copy(additionalData[2:], SequenceNumber[:])
	additionalData[8] = byte(ContentType)
	binary.BigEndian.PutUint16(additionalData[9:], uint16(Version))
